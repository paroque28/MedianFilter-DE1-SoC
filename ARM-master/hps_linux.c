...
void mmap_fpga_peripherals() {
    ...
    fpga_sdram = h2f_lw_axi_master + SDRAM_CONTROLLER_BASE + 0x2000;
}
...
void setup_fpga_sdram() {
    // Switch on first LED only
    alt_write_word(fpga_sdram, 0x1);
}

void handle_hps_sdram() {
...
void handle_fpga_leds() {
    uint32_t leds_mask = alt_read_word(fpga_sdram);

    if (leds_mask != (0x01 << (SDRAM_CONTROLLER_SDRAM_DATA_WIDTH - 1))) {
        // rotate leds
        leds_mask <<= 1;
    } else {
        // reset leds
    ...
    alt_write_word(fpga_sdram, leds_mask);
}
